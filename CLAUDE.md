...
- `app/lib/permissions.ts` — *who can do what*. `can(role, action)` checks a `RULES` table of `Action`s. Note the quirk: **Graphic Designer normalizes to Editor** (same permissions). This is the single source of truth for UI gating (create/edit/delete/review/manage). The logged-in user's role comes from `useMyRole()`, which looks it up in `team_members` by email. Current grants beyond Founder/Strategist: **Editor** has `create_ad`, `edit_title`, `edit_zone2`, `move_stage`, `review_ad`; **Media Buyer** has `edit_zone2`, `move_stage`, `edit_performance`. Deletion (`delete_ad`, `batch_delete`), strategy fields (`edit_zone1`), and `manage_lists` stay Founder + Strategist; `manage_team` is Founder-only. Two asymmetries worth knowing: Editors can delete *ideas* (the Ideas view gates those buttons on `create_ad`) but not *ads*, and an Editor can create an ad yet cannot fill the Zone 1 fields the Idea → Brief gate requires.
