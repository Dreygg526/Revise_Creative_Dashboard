...
**Fetch-on-mount means an open tab goes stale, and it gets reported as a bug.** There is no realtime subscription, no polling and no revalidation anywhere in the app — the only refetch is `AnalyticsView` calling `fetchAds()` after a Meta sync. A tab shows whatever existed when the view mounted, so work another person creates is invisible until the page reloads. `page.tsx` switches views with a `switch` that unmounts the old component, so navigating away and back *does* refetch — that's the quick workaround to hand people. Diagnosed 2026-08-19 when Axel reported OpenClaw could read ads #106–109 but the dashboard "couldn't see them": the ads had been created ~3 hours into his session, and the agent API queries live on every call while his tab did not. Nothing was broken. Expect this one again.
...
- **Read the anon-key warning at the top of Project status before treating any of the bounds below as security.** They bound what a *compromised agent key* can do, which is a real and worthwhile property. They do not bound what someone with the public anon key can do, which is currently everything.
...
Snapshot as of **2026-08-19**, re-verified against the tree that day (the measurements below keep their own dates). Update this when the situation changes; delete lines once they stop being true.

**⚠ The database is wide open to the public anon key. Measured 2026-08-19 — this is the biggest open issue in the project.** With `NEXT_PUBLIC_SUPABASE_ANON_KEY` and **no session at all**, an unauthenticated client can read `ads`, `team_members`, `settings_lists`, `settings_targets`, `meta_sync_runs` and `scripts`, and **write** `ads` — a `notes` update against a real row returned `1 row(s) written` (restored immediately; the row is unchanged). RLS is either disabled or has a permissive `anon` policy on these tables. That key is *designed* to be public — it ships in the client bundle of the deployed site, so anyone who opens devtools has it.

Consequences worth stating plainly:
- Every access control in this repo is UI-deep. `permissions.ts`, the pipeline gates, the session checks added to `invite` / `delete-member`, and the agent key's careful one-column write bound all guard the *app's* path to the data. None of them guard the database, which is reachable directly.
- **Fixing this is not a one-liner and must not be done casually.** Turning RLS on without first writing policies for the signed-in (`authenticated`) role locks every user out of a dashboard that reads these tables from the browser on every view. It needs policies written per table, then the whole app exercised while signed in.
- Until then, treat the deployed URL as the security boundary and don't put anything in this database you wouldn't hand to anyone who visits the site.
...
**Schema is applied.** All five SQL files (`meta_integration_schema.sql` → `_v2.sql` → `_v3.sql` → `_v4.sql` → `_v5.sql`) have been run against the live Supabase project — v1–v4 verified 2026-08-12, v5 verified 2026-08-13 (the breakdown panel renders populated). `agent_result_schema.sql` (independent of that chain) is applied too — both columns verified present 2026-08-19. Re-run them only when standing up a fresh database.
...
**Open decisions:**
- **Lock down the database with RLS policies.** See the warning at the top of this section. This is the highest-stakes item on the list and also the easiest to break the app with — policies have to be written per table for the `authenticated` role before RLS goes on, then the whole dashboard exercised while signed in. Recommended as the next piece of work, ahead of the break-even ROAS build.
- **Give the pipeline a way to refresh without a full reload.** A button in the board header, or a refetch when the window regains focus. Cheap, pure UI, and it removes a whole class of "the dashboard is missing ads" reports — see the fetch-on-mount note under the data layer.
...
Verified end-to-end against the live DB on DTC #1 (snapshot → three writes → exact restore, including `updated_at`): synonym normalization (`"Looser"` → `Killed`, `"win"` → `Winner`), `close: true` moving Testing → Winner / Killed, `previous_result` reporting, and clearing with `result: null`. Every error path returns the right status too. **`agent_result_schema.sql` is applied** (verified 2026-08-19) — a write now comes back `attribution_recorded: true` and stamps `result_source = 'agent'` + `result_set_at`. Both branches of the column probe were exercised: the run before the migration returned `attribution_recorded: false` with a warning and still wrote the verdict, which is the behaviour a future missing column should fall back to.
